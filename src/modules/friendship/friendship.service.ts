...
export class FriendshipService {
  ...
  async getUserFriendRequest(userId: number, search: string) {
    ...
    FROM ngtdb.friendship fs INNER JOIN ngtdb.user u ON fs.user_id = u.id WHERE ${query} fs.friend_id = ${userId} AND fs.isAccept = 0`);
  ...
  async saveMessageServer(
    ...
    return {
      id: saveMessage.id,
      server_id: saveMessage.server_id,
      user_id: saveMessage.user_id,
      message: saveMessage.message,
      image: saveMessage.image,
